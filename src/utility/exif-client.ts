...
// ---- Minimal piexifjs typings (remove if you install proper types) ----
// declare global {
//   // When loaded via <script src="https://unpkg.com/piexifjs"></script>
//   const piexif: {
//     load(dataURL: string): any;
//     dump(exifObj: any): string;
//     insert(exifBinaryOrDump: string, dataURL: string): string;
 ...
 */
export function extractExifFromPNG(arrayBuffer: ArrayBuffer): Uint8Array | null {
...
export async function insertExifIntoJpegBlob(
  ...
  const normalizedDataURL = piexif.insert(piexif.dump(exifObj), withExifDataURL);
 ...
 * - Honors EXIF orientation at decode using createImageBitmap(..., { imageOrientation: 'from-image' })
...
export async function pngToJpegWithExif(
  ...
  // 1) Extract EXIF from PNG (if present)
  const ab = await readAsArrayBuffer(file);
  const rawExif = extractExifFromPNG(ab); // Uint8Array | null

  // 2) Decode the PNG for drawing
  ...
  // TS lib.dom types include ImageBitmapOptions; the property is valid in modern browsers.
  const bitmap = await createImageBitmap(img, { imageOrientation: "from-image" } as any);
...
export function wireDemoInput(inputId = "png") {
  ...
  el.addEventListener("change", async () => {
    ...
    if (!file) return;
    try {
      const out = await pngToJpegWithExif(file, { maxDim: 1600, quality: 0.92 });
