        ...
        return <>
          ...
          >
            {aiActiveTextGenerationProvider === 'gateway' && <>
              Generating AI text via <strong>Vercel AI Gateway</strong>
              {aiGatewayActiveModel && <> ({aiGatewayActiveModel})</>}.
            </>}
            {aiActiveTextGenerationProvider === 'openai' && <>
              Generating AI text via the direct <strong>OpenAI</strong>
              {' '}integration.
            </>}
            {!isAiTextGenerationEnabled && <>
              AI text generation is <strong>off</strong>. Turn it on
              {' '}below — Vercel AI Gateway is the recommended path.
            </>}
          </ChecklistRow>
          <ChecklistRow
          ...
          >
            ...
            Store a{' '}
            {'"creator/model-name"'} string (e.g. {'"openai/gpt-5.2"'})
            to generate AI text via
            {' '}
            {renderLink(
            ...
            {' '}— cheaper vision tiers (e.g. openai/gpt-5-nano) are
            {' '}usually plenty for short captions.
            {isVercelDeployment
              ? <> No API key needed — Vercel authenticates automatically
                via OIDC.</>
              : <> Requires an API key outside Vercel, where authentication
                is not automatic.</>}
            {!hasAiGatewayModel && isVercelDeployment && <>
              {' '}
              <strong>
                On Vercel, so no key is required — set
                {' '}AI_GATEWAY_MODEL to turn it on.
              </strong>
            </>}
            {aiGatewayModel && <>
              {' '}(currently <strong>{aiGatewayModel}</strong>)
            </>}
            {hasAiGatewayModel
              && aiActiveTextGenerationProvider !== 'gateway'
              && <>
                {' '}— set, but <strong>OPENAI_SECRET_KEY</strong> takes
                {' '}precedence. Remove it to switch to the Gateway.
              </>}
          ...
          >
            ...
            Set <strong>OPENAI_SECRET_KEY</strong> to use OpenAI directly
            {' '}instead of the Gateway — when set, it takes precedence over
            {' '}<strong>AI_GATEWAY_MODEL</strong>. Enables every AI text
            {' '}field, including the invisible {'"Semantic Description"'}
            {' '}used for CMD-K search and image accessibility.
            {' '}
            Optionally set <strong>OPENAI_MODEL</strong>
            {' '}({'set to \'compatible\' to use gpt-4o'}) or
            {' '}<strong>OPENAI_BASE_URL</strong> for an OpenAI-compatible
            {' '}endpoint (experimental); both apply only while OpenAI is
            {' '}active.
            {renderEnvVars([
              'OPENAI_SECRET_KEY',
              'OPENAI_MODEL',
          ...
          >
            ...
            {!isAiTextGenerationEnabled && <>
              No provider configured (see the options above) — these
