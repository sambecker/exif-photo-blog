...
export const generatePhotoSyncStatus = (photo: PhotoDb): PhotoSyncStatus => ({
  ...
  isMissingColorData: CHROMATIC_SORT_ENABLED && (
    !photo.colorData ||
    !photo.colorHue
  ),
});
...
// TODO: add color data status text
export const getPhotoSyncStatusText = (photo: Photo) => {
  const { isOutdated, isMissingAiTextFields } = photo.syncStatus;
  const text: string[] = [];
  if (isOutdated) {
    text.push('Outdated Data');
  } else if (isMissingAiTextFields.length > 0) {
    const missingFieldsText = isMissingAiTextFields
